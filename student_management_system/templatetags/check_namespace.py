...
def has_namespace(request_url, args):
    print(
        "-----------------------I am inside Namespace-----------------------***Do this using session concept*******"
    )

    request_url_namespace = urls.resolve(request_url).namespace
    if len(args.split(",")) == 2:
        module_home_url, my_namespace = args.split(
            ",")  # args content multi variables
    # this is for some app that do not have namespace(i.e for schedule app or
    # others)
    else:
    ...
    # return True if request_url_namespace in list(all_namespace_list) else
    # False
