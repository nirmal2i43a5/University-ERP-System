...
class StudentSerializer(serializers.ModelSerializer):
    ...
    father_phone = serializers.CharField(
        source="guardian.father_phone", read_only=True)
    username = serializers.CharField(
        source="student_user.username", read_only=True)
    email = serializers.CharField(source="student_user.email", read_only=True)

    full_name = serializers.CharField(
        source="student_user.full_name", read_only=True)
    section = serializers.SerializerMethodField()

    semester = serializers.SerializerMethodField()

    def get_semester(self, obj):
        return f"{obj.semester.name}({obj.semester.course.course_name})"
