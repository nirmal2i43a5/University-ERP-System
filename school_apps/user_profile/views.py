...
from student_management_app.django_forms.forms import StaffForm, EditCustomUserForm
...
def change_other_password(request):
    ...
    context = {"title": "Reset Password", "reset_form": password_reset_form}
    return render(
        request,
        "user_reset_password/change_other_password.html",
        context)
...
def password_change_form(request):
    if request.method == "POST" and "change_pass_button" in request.POST:
        ...
        if PassForm.is_valid():
            ...
            messages.success(
                request,
                "Your password is successfully updated.Now you can login with your new password.",
            )
            return redirect("login")
...
def UpdateStudentProfile(request):
    ...
    return render(
        request,
        "profile/edit_student_profile.html",
        {
    ...
    if request.user.is_superuser:
        ...
        if request.method == "POST":  # and 'admin_submit' in request.POST:
            ...
            admin_form = SystemAdminForm(
                request.POST, request.FILES, instance=request.user.adminuser
            )
