...
def send_application_email(request, pk, email):
    ...
    html_content = render_to_string(
        "enquiry/application_form_email_template.html",
        {
            "enquiry": enquiry_obj,
        },
    )
    ...
    email = EmailMultiAlternatives(
        "Admission Form", text_content, settings.EMAIL_HOST_USER, [
            enquiry_obj.email])
...
def send_entrance_info(request, pk):
    ...
    html_content = render_to_string(
        "enquiry/entrance_info_email_template.html",
        {"enquiry": enquiry_obj, "date": date},
    )
    text_content = strip_tags(html_content)
...
def send_entrance_results(request, pk):
    ...
    date = request.POST["entrance_date"]
    html_content = render_to_string(
        "enquiry/entrance_result_email_template.html",
        {"enquiry": enquiry_obj, "date": date},
    )
    text_content = strip_tags(html_content)

    email = EmailMultiAlternatives(
        "Entrance Result", text_content, settings.EMAIL_HOST_USER, [
            enquiry_obj.email])
...
def send_interview_results(request, pk):
    enquiry_obj = Enquiry.objects.get(pk=pk)
    html_content = render_to_string(
        "enquiry/interview_result_email_template.html", {"enquiry": enquiry_obj}
    )
    text_content = strip_tags(html_content)

    email = EmailMultiAlternatives(
        "Interview Result", text_content, settings.EMAIL_HOST_USER, [
            enquiry_obj.email])
...
def send_confirmation_email(request, pk):
    enquiry_obj = Enquiry.objects.get(pk=pk)
    html_content = render_to_string(
        "enquiry/confirmation_email_template.html", {"enquiry": enquiry_obj}
    )
    text_content = strip_tags(html_content)
...
def application_form(request):
    ...
    if request.method == "POST":
        ...
        if admission_form.is_valid():
            ...
            messages.error(request, "Something went wrong.")
    return render(request, "enquiry/application_form.html",
                  {"form": app_form, "form_check": True})
...
def send_app_entrance_info(request, pk):
    ...
    email = EmailMultiAlternatives(
        "testemail", text_content, settings.EMAIL_HOST_USER, [
            enquiry_obj.email])
...
def app_enroll(request, pk):
    ...
    student_username = (
        str(join_year)
        + str(dept_id).zfill(2)
        + str(course_id).zfill(2)
        + str(student_count).zfill(4)
    )
    ...
    # return HttpResponse(str(student_count) + " " +
    # str(application_obj.course))
