...
class StudentAttendanceDetailsSearch(forms.Form):
    attendance_date = forms.DateField(
        label="",
        widget=forms.DateInput(attrs={"type": "date", "class": "col-md-12"}),
        initial=datetime.date.today,
    )
    ...
    semester = forms.ModelChoiceField(
        ...
        empty_label="Choose Class",
        queryset=Semester.objects.all(),
        widget=forms.Select(attrs={"class": "col-md-12"}),
    )
...
class StudentAttendanceEditDetailsSearch(forms.Form):
    edit_filter_attendance_date = forms.DateField(
        label="",
        widget=forms.DateInput(attrs={"type": "date", "class": "col-md-12"}),
        initial=datetime.date.today,
    )
    edit_flter_semester = forms.ModelChoiceField(
        label="", empty_label="Choose Class", queryset=Semester.objects.all()
    )
    edit_filter_group = forms.ChoiceField(
        required=False, label="", choices=faculty_choices
    )


# for respective student
class StudentAttendanceDateFilterForm(
    forms.Form
):  # for respective user not for taking attendance
...
# for respective student
class TeacherAttendanceDateFilterForm(
    forms.Form
):  # for respective user not for taking attendance
    # subject = forms.ModelChoiceField(empty_label = 'Select Subject',widget=forms.Select(attrs={}),
    #                                  queryset = Subject.objects.all())
...
class AttendanceDateFilterForm(
    forms.Form
):  # for respective user not for taking attendance
    start_date = forms.DateField(
        widget=forms.DateInput(attrs={"type": "date", "class": ""})
...
class FilterMonthlyAttendance(forms.Form):
    ...
    month = forms.ChoiceField(
        required=False, label="", choices=MONTH_CHOICES, initial=today_month
    )
    course_category = forms.ModelChoiceField(
        label="",
        empty_label="Choose Course Category",
