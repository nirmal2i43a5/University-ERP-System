...
class Question(models.Model):
    question_paper = models.ForeignKey(
        QuestionPaper, on_delete=models.CASCADE, null=True
    )
...
class SubQuestionGrade(models.Model):
    ...
    def __str__(self) -> str:
        return (
            str(self.student.student_user.full_name)
            + " "
            + str(self.sub_question.question_no)
            + " "
            + str(self.marks)
        )
...
class AnswerSheet(models.Model):
    questionpaper_status = (("Assigned", "Assigned"),
                            ("Completed", "Completed"))
    student = models.ForeignKey(Student, on_delete=models.CASCADE, null=True)
    exam = models.ForeignKey(Exams, on_delete=models.CASCADE, null=True)
    answer_upload = models.FileField(
        upload_to="Student_answers/student_answers", null=True, blank=True
    )
    graded_sheet = models.FileField(
        upload_to="Student_answers/graded_sheets", null=True, blank=True
    )
    questionpaper_status = models.CharField(
        choices=questionpaper_status,
        max_length=50,
        default="Assigned",
        null=True,
        blank=True,
...
class ExamAttendanceReport(models.Model):
    ...
    extra_user = models.ForeignKey(
        ExtraUser, on_delete=models.DO_NOTHING, null=True)
