...
from school_apps.exam.models import (
    QuestionPaper,
    Question,
    QuestionGrade,
    AnswerSheet,
...
class QuestionListView(View):
    def get(self, request, *args, **kwargs):
        ...
        # submitted_questionpaper.append(exam_obj.questionpaper.pk)#access
        # question(child) from parent instance exam
        ...
        return render(
            request,
            "exam/teachers/question_paper_list.html",
            context)
...
#         }
# return render(request, 'exam/teachers/add_each_question_mark.html',
# context)
...
def each_question_mark_create(request, questionpaper_id):
    SubQuestionFormset = modelformset_factory(
        SubQuestion, form=SubQuestionMark)
    ...
    if request.method == "POST":
        questionpaper_instance = get_object_or_404(
            QuestionPaper, pk=questionpaper_id)
    ...
    return render(
        request,
        "exam/teachers/add_each_question_mark.html",
        context)
...
class EachQuestionMarkView(View):
    def get(self, request, pk, *args, **kwargs):
        ...
        context = {"title": "Mark", "question_marks": question_marks}
        return render(
            request,
            "exam/teachers/eachquestion_mark_show.html",
            context)
...
def sub_question_marks_show(request, mainquestion_id):
    ...
    context = {"title": "Sub Question Mark",
...
def qustionpaper_mark_update(request, questionpaperid, pk):
    ...
    if request.method == "POST":
        ...
        questionpaper_instance = get_object_or_404(
            QuestionPaper, pk=questionpaperid)
...
class StudentQuestionPaperView(View):
    def get(self, request, *args, **kwargs):
        ...
        # Exclude questionpaper from student page which is already submitted
        student_assigned_papers = QuestionPaper.objects.filter(
            exam__semester=request.user.student.semester, draft=False
        ).exclude(pk__in=submitted_questionpaper)
        ...
        return render(
            request,
            "exam/students/student_question_paperview.html",
            context)
...
class StudentUploadAnswer(View):
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "exam/students/student_upload_answer.html",
            context)
        ...
        for item in student_list:
            try:
                application = application_form.objects.get(
                    student=item, term=student_answer_list[0].exam.term
                )
                valid_student_list.append(item)
            except BaseException:
                continue

        valid_answer_sheet = AnswerSheet.objects.filter(
            student__in=valid_student_list, exam_id=examid
        )  # show answersheet only for exam apply students
...
class TeacherAddGrade(View):
    def get(self, request, studentid, answerid, *args, **kwargs):
        ...
        grade_form.fields["question"].queryset = Question.objects.filter(
            question_paper__exam=answer_sheet.exam
        )
        ...
        for item in question:
            sub_question = sub_question | SubQuestion.objects.filter(
                question=item)
        ...
        question_grade_marks = QuestionGrade.objects.filter(
            student=Student.objects.get(pk=studentid),
            question__question_paper__exam=exam_instance,
        )
        sub_question_grade_marks = SubQuestionGrade.objects.filter(
            student=Student.objects.get(pk=studentid),
            sub_question__question__question_paper__exam=exam_instance,
        )
        ...
        for item in questions:
            try:
                q_marks = QuestionGrade.objects.get(
                    question=item, student=Student.objects.get(pk=studentid)
                )
                marks = q_marks.marks
                total_marks += marks
            except BaseException:
    ...
    def post(self, request, studentid, answerid, *args, **kwargs):
        ...
        for item in question:
            ...
            QuestionGrade.objects.create(
                student=student_instance,
                question=get_object_or_404(Question, pk=item.pk),
                marks=mark_instance,
            )
...
def add_student_grade(request):
    ...
    obj, created = QuestionGrade.objects.get_or_create(
        student=student_instance, question=question_instance
    )
    ...
    for item in questions:
        try:
            q_marks = QuestionGrade.objects.get(
                question=item, student=student_instance)
            marks = q_marks.marks
            total_marks += marks
        except BaseException:
            print(item, "marks not found")
    ...
    html = render_to_string(
        "exam/grades/show_grades_table.html",
        context=context)
    res = {"html": html}
    return JsonResponse(res, safe=False)

    # return render (request, 'exam/grades/show_grades_table.html',
    # context=context)
...
def add_student_grade_sub(request):
    ...
    obj, created = SubQuestionGrade.objects.get_or_create(
        student=student_instance, sub_question=subquestion_instance
    )
    obj.marks = marks
    obj.save()
    print(obj, "~~~~~~~~~~~~~~~~~~~~~~~~~~~", "OBJ FIRST")
    ...
    obj2, created2 = QuestionGrade.objects.get_or_create(
        student=student_instance, question=question_instance
    )
    ...
    for item in subquestions:
        try:
            print(
                "Subquestion: ",
                SubQuestionGrade.objects.get(
                    sub_question=item, student=student_instance
                ),
                "~~~~~~~~~~~~~~~~~~~~~~",
            )
            q_marks = SubQuestionGrade.objects.get(
                sub_question=item, student=student_instance
            )
            marks = q_marks.marks
...
def all_answer_upload(request):
    ...
    if request.method == "POST":  # and  'fetch_student' in request.POST :
        ...
        return render(
            request,
            "exam/admin/all_answer_upload.html",
            {"students": valid_students, "terms": Term.objects.all(), "exam": exam},
        )
    ...
    if request.method == "POST" and "upload_answer_button" in request.POST:
        ...
        for student_id, answer_file in zip(students, student_answer_files):
            ...
            obj, created = AnswerSheet.objects.get_or_create(
                exam_id=examid, student=student_instance
            )
    ...
    if request.method == "POST":
        ...
        try:
            answer_sheet = AnswerSheet.objects.get(
                exam=exam, student__student_user=request.user.id
            )
            print(
                answer_sheet,
                answer_sheet.student,
                answer_sheet.answer_upload,
                "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
            )
            context = {
                "exams": exams,
            ...
            return render(
                request,
                "exam/students/view_answersheet.html",
                context)
        except BaseException:
            context = {
                "exams": exams,
