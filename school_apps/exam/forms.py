...
class QuestionPaperUploadForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if user:
            ...
            subjects = SubjectTeacher.objects.filter(teacher=user).values_list(
                "subject", flat=True
            )
...
class QuestionMark(forms.ModelForm):
    ...
    total_marks = forms.FloatField(
        ...
        widget=forms.NumberInput(
            attrs={
                "placeholder": " Enter  Total Marks(Fill this Only if you do not have sub question)",
            }),
    )
    question_no = forms.CharField(
...
class SubQuestionMark(forms.ModelForm):
    question_description = forms.CharField(
        required=False, widget=forms.Textarea(attrs={"rows": 2, "cols": 10})
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "total_marks": forms.TextInput(
                attrs={"class": "formset-field sub_question_mark"}
            ),
            "question_description": forms.TextInput(
                attrs={"class": "formset-field", "rows": 3, "cols": 20}
            ),
        }
