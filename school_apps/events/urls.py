...
urlpatterns = [
    ...
    path('upcomingevents/',views.event_list_only,name = 'event_list_only'),
      path('upcomingholidays/',views.holiday_list_only,name = 'holiday_list_only'),
    ...
    path('event/delete/<pk>/',views.EventDeleteView.as_view(),name = 'event_delete'),
    url(r'^(?P<year>[0-9]{4})$', views.MonthlyCalendar.as_view(), kwargs={'month': 1}),
    url(r'^(?P<year>\d+)/(?P<month>[0-9][0-2]?)$', views.MonthlyCalendar.as_view(), name='calendar'),
    ...
    path('holiday-detail/<str:date>/',views.holiday_detail, name = 'holiday_detail'),
