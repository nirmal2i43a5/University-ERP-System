...
class ENoteForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if (
            user.is_superuser
            or user.groups.filter(name="Super-Admin")
            or user.groups.filter(name="Admin")
        ):
            course_category = self.fields["course_category"]
...
class AssignmentForm(forms.ModelForm):
    ...
    Subject = forms.ModelChoiceField(
        empty_label="Select Subject", queryset=Subject.objects.all()
    )
...
class ContentFilterForm(forms.Form):
    ...
    section = forms.ModelChoiceField(
        ...
        queryset=Section.objects.all())
    subject = forms.ModelChoiceField(
        empty_label="Choose Subject", queryset=Subject.objects.none()
    )
    ...
    def __init__(self, semester_id, *args, **kwargs):
        ...
        self.fields["section"].queryset = Section.objects.select_related(
            "course_category", "course", "semester", "staff"
        ).filter(semester=semester)
        self.fields["subject"].queryset = Subject.objects.select_related(
            "course_category", "course", "semester"
        ).filter(semester=semester)
...
class SubjectAssignFilterForm(forms.Form):
    course_category = forms.ModelChoiceField(
        label="",
        empty_label="Choose Course Category",
        queryset=CourseCategory.objects.all(),
    )

    filter_course = forms.ModelChoiceField(
        label="", empty_label="Choose Course", queryset=Course.objects.all()
    )

    filter_semester = forms.ModelChoiceField(
        label="", empty_label="Choose Class", queryset=Semester.objects.none()
    )
    section = forms.ModelChoiceField(
        required=False,
        label="",
...
class SubjectSearchForm(forms.Form):
    subject = forms.ModelChoiceField(
        label="", empty_label="Select Subject", queryset=Subject.objects.all()
    )

    def __init__(self, request, *args, user=None, **kwargs):
        super().__init__(*args, **kwargs)
        subject = self.fields["subject"]
        subject.queryset = subject.queryset.filter(
            staff_user__subject=request.user.student.subject
        )
...
class RoutineSearchForm(forms.Form):
    ...
    def __init__(self, *args, user, **kwargs):
        ...
        if user.is_superuser:
            self.fields["course_category"] = forms.ModelChoiceField(
                label="",
                required=False,
                empty_label="Course Category",
                queryset=CourseCategory.objects.all(),
            )
            self.fields["filter_course"] = forms.ModelChoiceField(
                label="",
                required=False,
                empty_label="Course ",
                queryset=Course.objects.all(),
            )
            self.fields["filter_semester"] = forms.ModelChoiceField(
                label="", empty_label="Select Semester", queryset=Semester.objects.all())
...
class SubjectForm(forms.ModelForm):
    ...
    semester = forms.ModelChoiceField(
        required=False, queryset=Semester.objects.all())
    course = forms.ModelChoiceField(
...
class SemesterForm(forms.ModelForm):  # for a-level
    course_category = forms.ModelChoiceField(
        queryset=CourseCategory.objects.exclude(course_name__in=["School"]),
        widget=forms.RadioSelect(),
    )
...
# attrs={'rows': 2, 'cols': 10, "placeholder": " Enter  Course
# Description", }))
...
class SectionForm(forms.ModelForm):
    ...
    # attrs={"class": "form-control", "placeholder": " Enter Section
    # Capacity", }))
