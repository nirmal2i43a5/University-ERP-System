...
class Enotes(models.Model):
    category = (("Subject Notes", "Subject Notes"),
...
class Assignment(models.Model):
    assignment_category = (("Assigned", "Assigned"),
                           ("Completed", "Completed"))
    course_category = models.ForeignKey(
        CourseCategory, on_delete=models.CASCADE, null=True, blank=True
    )
...
class Grade(models.Model):
    assignment_status = (("Assigned", "Assigned"), ("Completed", "Completed"))
    student = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
    assignment_status = models.CharField(
        _("Assignment Category"),
        choices=assignment_status,
        max_length=50,
        default="Assigned",
        null=True,
        blank=True,
    )  # For submitted status by the student
    assignment = models.ForeignKey(Assignment, on_delete=models.CASCADE)
    grade = models.PositiveIntegerField(null=True, blank=True)
    grade_status = models.BooleanField(
        default=False
    )  # for checking whether assignment is returned to student with points
    feedback = models.CharField(
        max_length=255, null=True, blank=True, default="No feedback"
    )
    answer_upload = models.FileField(
...
class Routine(models.Model):
    ...
    course = models.ForeignKey(
        ...
        null=True,
        blank=True)
    routine_file = models.FileField(
        _("Routine"),
        upload_to="College Routine",
        max_length=500,
        validators=[img_pdf_doc_validator],
    )
    semester = models.ForeignKey(Semester, on_delete=models.CASCADE)
    section = models.ForeignKey(
        Section, on_delete=models.CASCADE, blank=True, null=True
