...
def edit_transport(request, transport_id):
    ...
    if request.method == 'POST':
        form = TransportForm(request.POST, request.FILES, instance = transport_instance)
...
def delete_transport(request, transport_id):
    try:
        ...
        transport.delete()
        messages.success(request, f' Transport route {transport.route_name} is Deleted Successfully')
        return redirect('admin_app:manage_transport')
    except:
