...
from student_management_app.django_forms.forms import (
                                           AddCustomUserForm, DocumentFileForm
                                          )
from school_apps.parents.forms import (ParentForm, ParentCustomUserForm,)

from student_management_app.models import (
                                        	 CustomUser, Staff, Student, Parent  , DocumentFile, Student
                                           )
    ...
    if request.method == 'POST':
        ...
        if custom_form.is_valid() and parent_form.is_valid():
            ...
            try:
                ...
                user.parent.local_guardian_name = parent_form.cleaned_data['local_guardian_name']
                user.parent.mother_name = parent_form.cleaned_data['mother_name'].capitalize()
                user.parent.father_profession = parent_form.cleaned_data['father_profession']
                user.parent.mother_profession = parent_form.cleaned_data['mother_profession']
                ...
                user.parent.local_guardian_phone =  parent_form.cleaned_data['local_guardian_phone']
...
@permission_required('student_management_app.view_parent', raise_exception=True)
...
@permission_required('student_management_app.change_parent', raise_exception=True)
def edit_parent(request, parent_id):
    ...
    if request.method == 'POST':
        parent_form = ParentForm(request.POST, request.FILES, instance=parent_form_instance)
        custom_form = ParentCustomUserForm(request.POST, instance = custom_form_instance)
...
@permission_required('student_management_app.delete_parent', raise_exception=True)
def delete_parent(request, parent_id):
    try:
        parent = get_object_or_404(Parent, parent_user = parent_id)#i am using custom user so i use staff_user_id instead of normal parent id = staff_id
        parent.delete()
        messages.success(request, f'{parent.parent_user.username} is Deleted Successfully')
        return redirect('admin_app:manage_parent')
    except:
...
@csrf_exempt
@permission_required('student_management_app.add_parent_document', raise_exception=True)
def add_parent_document(request, parent_id):#for ajax part
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            #when i add_file then it must be the file of particular student with their respective id and i can retrieve this using _set see in view_student views function
            document.save()
            document_value=DocumentFile.objects.filter(parent=parent).values()#this filter data to show in ajax but commit filter for database
...
#         try:
#             parent = get_object_or_404(Parent, id = kwargs['parent_id'])#this is the  student_id  come from student not from customuser<check in view click>
...
@permission_required('student_management_app.view_parent_profile', raise_exception=True)
def view_parent(request, parent_id):
    try:
        parent = get_object_or_404(Parent, pk = parent_id)#this is the  student_id  come from student not from customuser<check in view click>
    except:
        return render(request, '404.html')
...
@csrf_exempt
@permission_required('student_management_app.edit_parent_document', raise_exception=True)
...
@permission_required('student_management_app.delete_parent_document', raise_exception=True)
def delete_parent_document(request,parent_id, document_id):
    try:
        document = get_object_or_404(DocumentFile, pk = document_id)#i am using custom user so i use staff_user_id instead of normal document id = staff_id
        document.delete()
