...
class studentgrades(models.Model):
    ...
    application_id = models.ForeignKey(
        application_form, on_delete=models.CASCADE, null=True, blank=True
    )
    marks = models.FloatField(default=0)
    grade = models.CharField(max_length=6, choices=GRADE_OPTIONS, default="U")
    passed = models.BooleanField()
    exam_type = models.BooleanField(
        null=True, blank=True)  # True=Regular, False=Chance
...
class selectedcourses(models.Model):
    student_id = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="course_student"
    )
    subject_id = models.ForeignKey(Subject, on_delete=models.CASCADE)
    year = models.IntegerField(
        choices=YEAR_CHOICES, default=datetime.datetime.now().year
    ...
    def __str__(self):
        return (self.student_id.student_user.full_name +
...
def create_exams_and_application_forms(sender, instance, created, **kwargs):
    if created:
        ...
        # filter(course_category = CourseCategory.objects.get(course_name = 'School'))
...
# students = Student.objects.all()#.values('student_user__username', 'pk',
# 'semester')
...
# app_obj.exam.add(item, through_defaults={'exam_type':True,
# 'passed':True})#
...
# return str(self.subject) + " semester:" + str(self.semester) + " day:" +
# str(self.day) + " period:" + str(self.period)
