...
from .helpers import manage_assignment, manage_enotes, manage_routine, manage_syllabus

school_course_category = get_object_or_404(
    CourseCategory, course_name="School")
plus_two_course_category = get_object_or_404(
    CourseCategory, course_name="Plus-Two")
...
plus_two_courses = Course.objects.select_related(
    "course_category", "department"
).filter(course_category=plus_two_course_category)
bachelor_courses = Course.objects.select_related(
    "course_category", "department"
).filter(course_category=bachelor_course_category)
master_courses = Course.objects.select_related(
    "course_category", "department").filter(
    course_category=master_course_category)

school_classes = Semester.objects.select_related(
    "course_category", "course").filter(
    course_category=school_course_category)
plus_two_classes = Semester.objects.select_related(
    "course_category", "course").filter(
    course_category=plus_two_course_category)
bachelor_classes = Semester.objects.select_related(
    "course_category", "course").filter(
    course_category=bachelor_course_category)
master_classes = Semester.objects.select_related(
    "course_category", "course").filter(
    course_category=master_course_category)
...
def attendance_chart_data(request, semester):
    ...
    present_status_count = (
        AttendanceReport.objects.select_related(
            "attendance", "student", "staff", "extra_user"
        )
        .filter(
            attendance__semester=semester_instance,
            status="Present",
    ...
    absent_not_informed_status_count = (
        AttendanceReport.objects.select_related(
            "attendance", "student", "staff", "extra_user"
        )
        .filter(
            attendance__semester=semester_instance,
            status="Absent(Not Informed)",
            attendance__attendance_date=datetime.today(),
        )
        .annotate(count=Count("status"))
        .count()
...
@permission_required(
    "student_management_app.view_superadmin_home", raise_exception=True
)
def classroom(request):
    url_name = resolve(request.path).url_name
    school_class_list = Semester.objects.select_related(
        "course_category", "course"
    ).filter(course_category=school_course_category)
    plus_two_class_list = Semester.objects.select_related(
        "course_category", "course"
    ).filter(course_category=plus_two_course_category)
    bachelor_class_list = Semester.objects.select_related(
        "course_category", "course"
    ).filter(course_category=bachelor_course_category)
    master_class_list = Semester.objects.select_related(
        "course_category", "course"
    ).filter(course_category=master_course_category)
    ...
    if url_name == "school-classroom":
        ...
        for semester in school_class_list:
            (
                ...
                absent_informed_status_count,
                absent_not_informed_status_count,
            ) = attendance_chart_data(request, semester)
            # Every eveery time function is called with respective semester
            student_attendance_dataset.append(
                {
        ...
        for semester in school_class_list:
            course, semester_instance, students = total_students_chart_data(
                request, semester
            )
            class_student_dataset.append(
                {
                    "course_name": course,
                    "semester_name": semester_instance,
                    "students": students,
                }
            )
        ...
        active_students = Student.objects.select_related(
            "student_user", "course_category", "semester", "section", "guardian").filter(
            course_category=school_course_category, student_user__is_active=True)
        inactive_students = Student.objects.select_related(
            "student_user", "course_category", "semester", "section", "guardian").filter(
            course_category=school_course_category, student_user__is_active=False)
        teachers = Staff.objects.prefetch_related("courses").filter(
            courses=school_course_category
        )
...
def classroom_contents(request, pk):
    ...
    (
        search_assignments,
        draft_assignments,
        assignment_search_form,
        assignment_submitted_status,
    ) = manage_assignment(request, pk)
    ...
    assignment_zip_file = (
        zip(search_assignments, assignment_submitted_status)
