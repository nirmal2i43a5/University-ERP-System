...
def assignment_handed_status(request):
    ...
    for assignment in (
        Assignment.objects.select_related(
            ...
            "section",
            "Subject",
            "teacher") .prefetch_related("grade_set") .all()):
        assignment_submitted_by_student = (
            assignment.grade_set.all()
        )  # filter(assignment_status = 'Completed').count()
        ...
        ).aggregate(review_count=Count("pk"))["review_count"]
...
def manage_assignment(request, semester_id):
    semester_instance = (
        Semester.objects.filter(
            pk=semester_id).values_list(
            "id",
            flat=True).first())
    draft_assignments = Assignment.objects.select_related(
        "course_category",
        "course",
        "semester",
        "section",
    ...
    submitted_assignments = Grade.objects.select_related(
        "student", "assignment"
    ...
    assignment_search_form = forms.ContentFilterForm(
        semester_id,
        initial={
    ...
    if subject_id and start_date and end_date:
        ...
        search_assignments = Assignment.objects.select_related(
            "course_category",
            "course",
            "semester",
            "section",
    ...
    if subject_id:
        search_assignments = Assignment.objects.select_related(
            "course_category", "course", "semester", "section", "Subject", "teacher"
        ).filter(
            Subject=subject_instance,
...
def manage_syllabus(request, semester_id):
    ...
    search_syllabus = Syllabus.objects.select_related(
        "course", "semester", "subject"
    ).filter(semester=semester_instance)
    ...
    if subject_instance:
        search_syllabus = Syllabus.objects.select_related(
            "course", "semester", "subject"
        ).filter(semester=semester_instance, subject=subject_instance)
...
def manage_routine(request, semester_id):
    ...
    search_routines = Routine.objects.select_related(
        "course_category", "course", "semester", "section"
    ).filter(semester=semester_instance)
    ...
    if section_instance:
        search_routines = Routine.objects.select_related(
            "course_category", "course", "semester", "section"
        ).filter(semester=semester_instance, section=section_instance)
...
def manage_enotes(request, semester_id):
    ...
    if subject_id or category or start_date or end_date:
        if start_date and end_date:
            ...
            search_enotes = Enotes.objects.select_related(
                "course_category", "course", "semester", "section", "subject"
            ).filter(
                subject=subject_instance,
