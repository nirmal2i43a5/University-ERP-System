...
def teacher_send_bulk_email(request):
    teacher_bulk_email = []
    teachers_email = Staff.objects.values_list("staff_user__email", flat=True)
...
def particular_user_email(request):
    ...
    return render(
        request, "bulk_email/particular_user.html", {"title": "User  Message"}
    )
...
def send_sms_ajax(request):
    ...
    r = requests.post(
        ...
        data={"username": "info@gci.edu.np", "password": "Global@123@info"},
    )
