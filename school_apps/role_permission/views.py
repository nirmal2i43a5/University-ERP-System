...
def user_permission_manage(request):
    ...
    if role:
        ...
        permission_instance_form = ShowRolePermission(
            instance=group
        )  # showing permission form instance
        form = UserPermissionSearch(
            initial={"role": role}
        )  # this show instance in form even after post search form
...
def save_permission(request):
    if request.method == "POST":
        ...
        permission_instance_form = ShowRolePermission(
            instance=group, data=request.POST
        )  # showing permission form instance
