...
@permission_required("student_management_app.add_teacher",
...
@permission_required("student_management_app.view_teacher",
                     raise_exception=True)
def manage_teacher(request):
    staffs = Staff.objects.select_related("staff_user", "department").all()

    context = {"staffs": staffs, "title": "Manage Teacher"}

    return render(request, "teachers/manage_staff.html", context)


@permission_required("student_management_app.change_teacher",
                     raise_exception=True)
def edit_teacher(request, staff_id):
    ...
    if request.method == "POST":
        staff_form = StaffForm(
            request.POST, request.FILES, instance=staff_form_instance
        )
...
@permission_required("student_management_app.delete_teacher",
                     raise_exception=True)
def delete_teacher(request, staff_id):
    try:
        ...
        staff.delete()
        messages.success(
            request, f"{staff.staff_user.username} is Deleted Successfully"
        )
        return redirect("admin_app:manage_staff")
    except BaseException:
...
def search_teacher(request):
    ...
    if search_text:
        search_staffs = Staff.objects.filter(
            Q(staff_user__first_name__icontains=search_text)
            | Q(contact__iexact=search_text)
        )
        return render(request,
                      "teachers/manage_staff.html",
                      {"staffs": search_staffs})
    else:
...
@csrf_exempt
@permission_required(
    "student_management_app.add_teacher_document", raise_exception=True
)
...
# filter(staff = teacher_id, attendance__in = attendance)# or
# filter(student = student_id, attendance__attendance_date = month)
...
@permission_required(
    "student_management_app.view_teacher_profile", raise_exception=True
)
def view_teacher(request, teacher_id):
    try:
        # this is the  student_id  come from student not from customuser<check
        # in view click>
        teacher = Staff.objects.get(staff_user=teacher_id)
        # this retrieve subjects belongs to customuser teacher
        customuser_teacher = CustomUser.objects.filter(
            user_type=Group.objects.get(name="Teacher")
        ).get(id=teacher_id)
...
@csrf_exempt
@permission_required(
    "student_management_app.edit_teacher_document", raise_exception=True
)
...
@permission_required(
    "student_management_app.delete_teacher_document", raise_exception=True
)
def delete_teacher_document(request, teacher_id, document_id):
    try:
        ...
        document = get_object_or_404(DocumentFile, id=document_id)
        document.delete()
...
def student_view_by_teacher(request):
    ...
    semester_instance = (
        Semester.objects.get(pk=semester_query) if semester_query else None
    )
    section_instance = Section.objects.get(
        pk=section_query) if section_query else None
    ...
    if semester_query and section_query:
        search_students = Student.objects.select_related(
    ...
    if semester_query:
        search_students = Student.objects.select_related(
            "student_user", "semester", "section", "guardian"
        ).filter(semester=semester_instance, student_user__is_active=1)
        context = {"students": search_students, "form": search_form}
        return render(request, "teachers/students/student_list.html", context)
