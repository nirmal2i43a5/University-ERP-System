...
def teacher_home(request):
    ...
    # teacher = get_object_or_404(Staff, staff_user = request.user)#for
    # current login teacher
    subjects = Subject.objects.filter(
        staff_user=request.user.id
    )  # sub for current login user
    # subject belongs to particular teacher and subject is fk in student.so
    # access
