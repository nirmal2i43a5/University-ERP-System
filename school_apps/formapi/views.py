...
def api_update_view(request, pk):
    ...
    serializer = formTemplateSerializer(
        selected_formTemplate, data=request.data)
    data = {}
    if serializer.is_valid():
...
def formbuilder_select(request):
    ...
    return render(
        request,
        "formapi/form_builder_dept_select.html",
        context=context)
...
def admissionform_select(request):
    ...
    return render(
        request,
        "formapi/admission_form_dept_select.html",
        context=context)
...
def edit_form(request, pk):
    ...
    token = Token.objects.get(user=request.user.id).key
    context = {"form_id": form_id, "dept_id": dept_id, "token": token}
