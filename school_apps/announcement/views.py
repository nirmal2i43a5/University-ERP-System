...
def manage_holiday(request):
    ...
    return render(
        request,
        "announcements/holidays/manage_holiday.html",
        context)
    ...
    if request.method == "POST":
        ...
        try:
            if form.is_valid():
                ...
                try:
                    # if one notice is already active then it will become
                    # inactive if i add new one
...
def update_notice(request):
    ...
    return render(
        request, "announcements/notices/notice_list.html", {"notices": notice_data}
    )  # because of design fluctuation i am rendering to notice_list.html
