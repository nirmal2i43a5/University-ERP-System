...
def add_routine(request):
    if request.method == "POST":
        ...
        try:
            if form.is_valid():
                ...
                create_notification(
                    request,
                    post=f"Routine is added for {section}",
                    notification_type=1,
    ...
    context = {"form": form, "title": "Routine"}
...
def manage_routine(request):
    ...
    if semester and section:
        search_routines = Routine.objects.filter(
            semester=semester, section=section)
        context = {
            "routines": search_routines,
            "form": RoutineSearchForm(
                user=request.user, initial={"semester": semester}
        ...
        return render(
            request,
            "academic/routines/manage_routine.html",
            context)
    ...
    context = {"form": search_form, "routines": routines, "title": "Routine"}
...
def view_student_routine(request):
    routine = Routine.objects.filter(
        ...
        section=request.user.student.section).first()
    context = {
        "routine": routine,
        #    'routines': routines,
        "title": "Routine",
    ...
    return render(
        request,
        "academic/routines/view_student_routine.html",
        context)
