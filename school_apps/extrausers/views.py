...
@permission_required("student_management_app.add_extrauser",
...
@permission_required("student_management_app.view_extrauser",
                     raise_exception=True)
...
@permission_required("student_management_app.change_extrauser",
                     raise_exception=True)
def edit_user(request, user_id):
    ...
    if request.method == "POST":
        extra_user_form = ExtraUserForm(
            request.POST, request.FILES, instance=extra_user_form_instance
        )
        ...
        try:
            if extra_user_form.is_valid() and custom_form.is_valid():
                ...
                custom_form.save()
                messages.success(
                    request, f"Successfully Edited {custom_form_instance.username}")
                return redirect("admin_app:manage_user")

        except BaseException:
            messages.error(
                request, f"Failed to Update {custom_form_instance.username}")
...
def delete_user(request, user_id):
    try:
        # i am using custom user so i use staff_user_id instead of normal staff
        # id = staff_id
        user = get_object_or_404(ExtraUser, extra_user=user_id)
        user.delete()
        messages.success(
            request,
            f"{user.extra_user.username} is Deleted Successfully")
        return redirect("admin_app:manage_user")
    except BaseException:
...
@csrf_exempt
@permission_required(
    "student_management_app.add_extrauser_document", raise_exception=True
)
...
@permission_required(
    "student_management_app.view_extrauser_profile", raise_exception=True
)
...
@csrf_exempt
@permission_required(
    "student_management_app.edit_extrauser_document", raise_exception=True
)
...
@permission_required(
    "student_management_app.delete_extrauser_document", raise_exception=True
)
