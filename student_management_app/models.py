...
from student_management_system.validators import (
    validate_file_extension,
    img_pdf_file_validate_file_extension,
    pdf_file_validate_file_extension,
)
...
class CustomUser(
        AbstractUser):  # use this for extending deafult django auth system
...
class CourseCategory(
        models.Model):  # i also want to see subject for particular course
...
class AdminUser(models.Model):
    # linking main auth model to admin which assists to relate to admin table
    # with its id.Do same for staffs and students
...
class ExtraUser(
        models.Model):  # this all other user like driver accountant and so on
...
class Semester(models.Model):
    ...
    staff_user = models.ManyToManyField(
        CustomUser, through="SemesterTeacher", blank=True
    )
...
class Subject(models.Model):
    faculty_choices = (("Science", " Science"),
                       ("Non-Science", " Non-Science"))
    # I manually created subject because to replace subject_code pk to
    # subject_id
    ...
    faculty = models.CharField(
        _("Group"), max_length=50, choices=faculty_choices, blank=True
    )
    subject_name = models.CharField(max_length=255)
    subject_code = models.CharField(max_length=255, unique=True)
    staff_user = models.ManyToManyField(
        CustomUser, through="SubjectTeacher", blank=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now_add=True)
    ...
    # def __str__(self):
    # return f'{self.semester} : {self.subject_name} : ' + ' ,
    # '.join([str(teacher) for teacher in self.staff_user.all()])
...
class Section(models.Model):
    ...
    semester = models.ForeignKey(
        Semester, verbose_name="Class", on_delete=models.CASCADE, null=True
    )
        ...
        constraints = [
            models.UniqueConstraint(
                fields=[
                    "subject",
                    "section"],
                name="unique_subject_teacher_section")]

    def __str__(self):
        if self.subject.course:
            return (
                f"{self.subject.subject_name} : ( {self.subject.course.course_name} )"
            )
        else:
...
class Parent(models.Model):
    ...
    )  # full_namei.e customuser retrieve from this is placed in father name
...
class StudentGroup(models.Model):
    name = models.CharField(
        max_length=250,
        blank=True,
        verbose_name="Student Union Name",
        help_text="This is Student  Union GrouP",
    )
...
class Student(models.Model):
    ...
    shift = (("Morning", "Morning"), ("Day", "Day"))
    status_choices = (("Running", "Running"), ("In Active", "In Active"))
    faculty_choices = (("Science", "Science"), ("Non-Science", "Non-Science"))
    ...
    gender = models.CharField(
        max_length=100, choices=gender_choice, null=True, default="Male"
    )
    ...
    guardian = models.ForeignKey(
        Parent, on_delete=models.DO_NOTHING, null=True, blank=True
    )
    ...
    blood_group = models.CharField(
        max_length=25, choices=blood_group_choices, blank=True
    )
    optional_subject = models.ForeignKey(
        Subject,
        related_name="student_optional_subject",
        on_delete=models.CASCADE,
        null=True,
    )
    ...
    )  # I am using attendance based on subject
...
# return f'employee documents / {instance.employee_id} .
...
class DocumentFile(models.Model):
    ...
    file = models.FileField(
        upload_to="document_files",
        max_length=500,
        validators=[img_pdf_file_validate_file_extension],
    )
...
class CertificateTemplate(models.Model):
    ...
    photo = models.ImageField(
        upload_to="student_character_certificates_photos",
        blank=True,
        null=True)
    created_at = models.DateField(auto_now_add=True)
    updated_at = models.DateField(auto_now=True)
    ...
    background_image = models.ImageField(
        upload_to="certificate_template_background", blank=True
    )
